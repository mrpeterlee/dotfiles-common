...
def render_ssh_config(hosts: list[Host], role_filter: str | None = None) -> str:
    ...
    - ``hostnames[]`` legacy aliases are merged into the OpenSSH multi-host
      ``Host`` line (e.g. ``Host acap-sg-prod-1 sg-prod-1``) so legacy SSH
      bookmarks keep working through the rename window required by P4 T11.
    """
    lines = [
    ...
    for h in hosts:
        if role_filter and h.role != role_filter:
            continue
        host_aliases = " ".join(h.hostnames)
        host_line = f"Host {h.name}" + (f" {host_aliases}" if host_aliases else "")
