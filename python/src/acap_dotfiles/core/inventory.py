...
def render_ssh_config(hosts: list[Host], role_filter: str | None = None) -> str:
    ...
    Note: ``hostnames[]`` aliases are intentionally NOT merged into a
    multi-host ``Host`` line. Round-2 attempted that, but codex round-3 P1
    caught it: in the existing inventory, aliases like ``oracle-a``,
    ``oracle-b``, ``acap-reality-1``, ``acap-admin`` need DIFFERENT
    user/key/port/ProxyJump than the canonical host — they are NOT just
    "old names for the same host". Folding them into one ``Host`` block
    forces them all to share the canonical's SSH params, breaking
    ``ssh oracle-a`` etc. Legacy alias preservation is a P4 T11 concern
    that needs a richer model (per-alias overrides on ``Host``, or
    separate inventory entries for divergent aliases).
    """
    lines = [
